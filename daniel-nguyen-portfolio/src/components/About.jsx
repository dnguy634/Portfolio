...
export default function About() {
  return (
    <Section
      ...
      description="I'm a Computer Science + Business student at UC Riverside who builds where engineering and execution meet. I write the code, but I also scope the requirements, manage the timeline, and talk to the stakeholders who depend on the outcome — that's the combination I bring to product and program teams."
    >
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {stats.map((stat, i) => (
          <Reveal key={stat.label} delay={i * 0.08}>
            <GlassCard className="h-full p-6">
              <p className="font-display text-2xl font-semibold text-ink-100">{stat.value}</p>
              <p className="mt-2 text-sm font-medium text-ink-300">{stat.label}</p>
              <p className="mt-1 text-xs text-ink-700">{stat.hint}</p>
            </GlassCard>
          </Reveal>
        ))}
      </div>
